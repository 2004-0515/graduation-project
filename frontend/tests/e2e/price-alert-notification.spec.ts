...
test('降价提醒通知可跳转到商品详情', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /降价提醒/ }).click()
  await page.locator('.alert-dialog input[type="number"]').fill(String(targetPrice))
  await page.locator('.alert-dialog-footer .btn-primary').click()
