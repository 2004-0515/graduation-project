...
test('音乐播放器展示数据必须与本地音频文件一致', async ({ page }) => {
  ...
  for (const music of musicList) {
    const filename = filenameFromUrl(music.url)
    expect(filename).not.toContain('shion-light-')
    expect(placeholderTitles.has(music.title), `旧占位歌名仍在接口中: ${music.title}`).toBe(false)
    expect(music.artist).not.toBe('山川音室')
    const expected = expectedByFile[filename]
